#!/usr/bin/env python3
"""Generate th/id/ms translation data from unique EN strings (fast, cached)."""
import json
import re
import time
...
def main() -> None:
    ...
    for loc, code in LOCALES.items():
        ...
        locale_maps[loc] = translate_all(unique_en, loc, code)
        (OUT_DIR / f"{loc}-strings.json").write_text(
            json.dumps(locale_maps[loc], ensure_ascii=False, indent=2) + "\n",
            encoding="utf-8",
        )
        log(f"saved {loc}-strings.json")

    for loc in LOCALES:
        by_page: dict[str, dict[str, str]] = {}
        for item in entries:
            by_page.setdefault(item["pageId"], {})[item["key"]] = locale_maps[loc][item["en"]]
